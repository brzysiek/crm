"""Testy numeracji ofert M&A („Ref: <nr w miesiącu>/<miesiąc>/<rok>”) — baza jest zamockowana.
...
class NextOfferRefTest(unittest.TestCase):
    def setUp(self):
        ...
        # Model filtruje po LIKE '%/MM/RRRR' — odtwarzamy to na liście w pamięci.
    ...
    def test_first_offer_in_month_starts_at_one(self):
        self.assertEqual(self.ref(), "Ref: 1/09/2026")

    def test_month_is_zero_padded(self):
        self.assertEqual(self.ref(date(2026, 3, 1)), "Ref: 1/03/2026")

    def test_numbering_continues_after_highest(self):
        self.stored = ["Ref: 1/09/2026", "Ref: 2/09/2026", "Ref: 3/09/2026"]
        self.assertEqual(self.ref(), "Ref: 4/09/2026")
    ...
    def test_gap_does_not_reuse_number(self):
        ...
        self.stored = ["Ref: 1/09/2026", "Ref: 3/09/2026"]
        self.assertEqual(self.ref(), "Ref: 4/09/2026")

    def test_other_months_do_not_count(self):
        self.stored = ["Ref: 7/08/2026", "Ref: 9/09/2025"]
        self.assertEqual(self.ref(), "Ref: 1/09/2026")

    def test_numbering_restarts_each_month(self):
        self.stored = ["Ref: 12/09/2026"]
        self.assertEqual(self.ref(date(2026, 10, 5)), "Ref: 1/10/2026")

    def test_ignores_manually_edited_refs(self):
        self.stored = ["Ref: 1/09/2026", "Ref: aneks/09/2026"]
        self.assertEqual(self.ref(), "Ref: 2/09/2026")

    def test_ref_fits_in_column(self):
        self.stored = ["Ref: 998/12/2026"]
...
class CreateOfferRefTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cursor.fetchall.return_value = [{"ref_number": "Ref: 2/09/2026"}]
    ...
    def test_missing_ref_is_generated(self):
        ...
        self.assertEqual(params[1], f"Ref: 3/{date.today():%m/%Y}")

    def test_given_ref_is_kept(self):
        crm_mna_offer.create_mna_offer({"name": "Firma X", "ref_number": "Ref: 99/01/2020"}, user_id=1)
        self.assertEqual(self.insert_call().args[1][1], "Ref: 99/01/2020")
