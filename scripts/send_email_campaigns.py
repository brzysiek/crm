...
def run():
    with app.app_context():
        ...
        if not sender_email or not api_token:
            print('Brak konfiguracji wysyłki (adres nadawcy / token konta usługi) — Ustawienia → Ogólne.')
            return
        ...
        for i, recipient in enumerate(batch):
            ...
            if is_unsubscribed(recipient['email']):
                ...
                print(f"Pominięto (wypisany): {recipient['email']}")
                continue
            ...
            try:
                ...
                print(f"Wysłano: {recipient['email']} (id={message_id})")
            except Exception as e:
                mark_failed(recipient['id'], str(e))
                print(f"Błąd wysyłki do {recipient['email']}: {e}")
